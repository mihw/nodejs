...
describe('Vuex Store', () => {
  ...
  beforeEach(() => {
    ...
    store = new Vuex.Store({
      ...
      mutations: {
        setWndStatuses: (state: StoreState, payload: any) => {
        ...
        moveWndToTop: (state: StoreState, payload: any) => {
          ...
          for (let key in state.wndStatuses) {
            if ((state.wndStatuses[key].zIndex > oldZIndex) && (key !== String(payload.wndID))) {
      ...
      actions: {
        setWndStatuses (context: any, payload: any) {
          context.commit('setWndStatuses', payload)
        },
        moveWndToTop (context: any, payload: any) {
